...
with
  (security_invoker = true) AS (
    select
      ...
      CAST(
        grouped_pledges.total_sum_unfulfilled_pledges as INTEGER
      ),
      CAST(
        grouped_pledges.number_of_unfulfilled_pledges as INTEGER
      ),
    ...
    from
      ...
      full outer join (
        select
          sum(amount) as total_sum_unfulfilled_pledges,
